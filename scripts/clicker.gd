...
var reload = preload("res://assets/sounds/reload.mp3")
...
@onready var weapon_sniper = get_node_or_null("/root/Node2D/WeaponSniper")
@onready var target = get_node_or_null("/root/Node2D/Target")
...
func _physics_process(delta):
	...
	if operate:
		...
		elif Input.is_action_just_pressed("weapon_throw"):
			Global.weapon_mode = 1
		elif Input.is_action_just_pressed("weapon_bat") and Global.unlocked.has(2):
			Global.weapon_mode = 2
		elif Input.is_action_just_pressed("weapon_pistol") and Global.unlocked.has(3):
			Global.weapon_mode = 3
		elif Input.is_action_just_pressed("weapon_ak") and Global.unlocked.has(4):
			Global.weapon_mode = 4
		elif Input.is_action_just_pressed("weapon_sniper") and Global.unlocked.has(5):
			Global.weapon_mode = 5
		
		if !(Global.weapon_mode == 2 or Global.weapon_mode == 1):
		...
		if weapon_sniper:
			weapon_sniper.visible = Global.weapon_mode == 5
		...
		if intersection && Input.is_action_just_pressed("click"):
			clicked = true
			if Global.weapon_mode == 0 or Global.weapon_mode == 3 or (Global.weapon_mode == 5 and sniper_mode):
				if Global.weapon_mode == 3:
					Global.score += 5
				elif Global.weapon_mode == 5:
					Global.score += 20
				else:
					Global.score += 1
		...
		if Input.is_action_just_pressed("click") and Global.weapon_mode == 5:
			sniper_mode = !sniper_mode
			if sniper_mode:
				var audio_player = audio.instantiate()
				audio_player.stream = reload
		...
		if self.position.y < bound:
				...
				if collision and collision.get_collider().name.contains("Border"):
					...
					if collision_normal.y > 0.5:
						...
						if Global.weapon_mode == 1:
							...
							Global.score += 5 * (Global.wallBounceCount + 1)
						else:
							Global.score += 1
			...
			if !grounded:
				...
				audio_player.stream = thud
				audio_player.pitch_scale = randf_range(0.9, 1.1)
				audio_player.volume_db = clamp(abs(velocity.y)/400, 0, 50.0)
				add_child(audio_player)
			velocity = Vector2.ZERO
